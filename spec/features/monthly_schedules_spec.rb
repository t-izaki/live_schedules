...
describe 'MonthlySchedules', type: :feature do
  ...
  describe '#edit->#update' do
    describe 'monthly_schedulesが更新できる' do
      ...
      let!(:performer) { schedule.performers.first }
      let!(:new_performer) { create(:performer) }
      ...
      it '更新なし' do
        click_on '更新する'
        expect(page).to have_http_status(:success)
        expect(page).to have_content("2018-01-10 水 open #{performer.name} ピアノ演奏会 ピアノの演奏会です")
      end

      it '状態が更新できる' do
        select 'closed', from: "schedule_#{schedule.id}_state"
        click_on '更新する'
        expect(page).to have_content("2018-01-10 水 closed #{performer.name} ピアノ演奏会 ピアノの演奏会です")
      end
      ...
      it 'タイトルが更新できる' do
        ...
        click_on '更新する'
        expect(page).to have_content("2018-01-10 水 open #{performer.name} バイオリン演奏会 ピアノの演奏会です")
      end

      it '詳細が更新できる' do
        fill_in "schedule_#{schedule.id}_detail", with: 'バイオリンの演奏会です'
        click_on '更新する'
        expect(page).to have_content("2018-01-10 水 open #{performer.name} ピアノ演奏会 バイオリンの演奏会です")
      end
    end
  end
end
